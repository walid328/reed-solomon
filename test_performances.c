...
int time_encode(int block_length, int *message_lengths, int qty, array *messages)
{
	long long start = timeInMilliseconds();
	for (int i = 0; i < qty; i++)
		array_free(rs_encode_2(block_length, message_lengths[i], messages[i]));
	return timeInMilliseconds() - start;
}

int time_fast_encode(int block_length, int *message_lengths, int qty, array *messages)
{
	long long start = timeInMilliseconds();
	for (int i = 0; i < qty; i++)
		array_free(rs_fast_encode(block_length, message_lengths[i], messages[i]));
	return timeInMilliseconds() - start;
}

int time_decode(poly g_0, int block_length, int *message_lengths, int qty, array *receiveds)
{
	long long start = timeInMilliseconds();
	for (int i = 0; i < qty; i++)
		array_free(rs_decode_2(g_0, block_length, message_lengths[i], receiveds[i]));
	return timeInMilliseconds() - start;
}

int time_fast_decode(poly g_0, int block_length, int *message_lengths, int qty, array *receiveds)
{
	long long start = timeInMilliseconds();
	for (int i = 0; i < qty; i++)
		array_free(rs_fast_decode(g_0, block_length, message_lengths[i], receiveds[i]));
	return timeInMilliseconds() - start;
}
...
void comp_time_encode(int qty, int *time1, int *time2)
{
	...
	for (int i = 0; i < qty; i++)
	{
		...
		messages[i] = array_new_rand(message_lengths[i]);
	}
...
void comp_time_decode(int qty, int *time1, int *time2)
{
	...
	{
		message_lengths[i] = rand_range(1, n - 1);
		receiveds[i] = rs_encode_2(n, message_lengths[i], array_new_rand(message_lengths[i]));
		array_add_errors(receiveds[i], n, rand_range(0, (n - message_lengths[i]) / 2));
	}
...
{
	FILE *fptr_results;
	fptr_results = fopen("results.csv", "w");
	assert(fptr_results);

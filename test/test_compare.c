...
bool comp_dft(int qty)
{
	for (int dega = -1; dega < n; dega++)
	{
		for (int i = 0; i < qty; i++)
		{
			...
			while (d <= n)
			{
				zp_array eval1 = poly_dft(pa, d);
				zp_array eval2 = poly_fft(pa, d);
				if (!zp_array_equal(eval1, eval2, d))
				{
					printf("Execution stopped for values:\n");
					poly_print(pa);
					zp_array_print(eval1, d);
					zp_array_print(eval2, d);
					return false;
				}
				zp_array_free(eval1);
				zp_array_free(eval2);
...
bool comp_inv_dft(int qty)
{
	for (int d = 1; d <= n; d *= 2)
	{
		for (int i = 0; i < qty; i++)
		{
			zp_array eval = zp_array_new_rand(d);
			poly pa = poly_new();
			poly pb = poly_new();
...
bool comp_encode(int qty)
{
	for (int block_length = 2; block_length <= n; block_length *= 2)
	{
		for (int message_length = 1; message_length < block_length; message_length++)
		{
			for (int i = 0; i < qty; i++)
			{
				zp_array message = zp_array_new_rand(message_length);
				zp_array codeword1 = rs_encode_2(block_length, message_length, message);
				zp_array codeword2 = rs_fast_encode(block_length, message_length, message);
				if (!zp_array_equal(codeword1, codeword2, block_length))
				{
					printf("Execution stopped for values:\n");
					zp_array_print(message, message_length);
					zp_array_print(codeword1, block_length);
					zp_array_print(codeword2, block_length);
					return false;
				}
				zp_array_free(message);
				zp_array_free(codeword1);
				zp_array_free(codeword2);
			}
		}
	}
	return true;
	...
	for (int block_length = 2; block_length <= n; block_length *= 2)
	{
		...
		for (int message_length = 1; message_length < block_length; message_length++)
		{
			for (int number_errors = 0; number_errors <= (block_length - message_length) / 2; number_errors++)
			{
				...
				{
					zp_array message = zp_array_new_rand(message_length);
					zp_array received = rs_fast_encode(block_length, message_length, message);
					zp_array_add_errors(received, block_length, number_errors);
					zp_array decode1 = rs_decode_2(g_0, block_length, message_length, received);
					zp_array decode2 = rs_fast_decode(g_0, block_length, message_length, received);
					if (!zp_array_equal(decode1, message, message_length) || !zp_array_equal(decode1, decode2, message_length))
					{
						printf("Execution stopped for values:\n");
						zp_array_print(message, message_length);
						zp_array_print(received, block_length);
						zp_array_print(decode1, message_length);
						zp_array_print(decode2, message_length);
						return false;
					}
					zp_array_free(message);
					zp_array_free(received);
					zp_array_free(decode1);
					zp_array_free(decode2);
				}
			}
		}
		poly_free(g_0);
